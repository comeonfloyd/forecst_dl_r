...
df_hours <- rename_hours(df_hours)
...
# for(i in 5:ncol(df_master)) {
#   df_master$i <- df_master[, i] / df_master[, 31]
# }

# Считаем удельные значения от часов
df_test <- df_master

for(i in 5:ncol(df_test)) {
  df_test[,  i] <- df_test[, i] / df_test$Часы_СП
}
